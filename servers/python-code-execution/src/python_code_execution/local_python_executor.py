...
# This file is modified from the original file in the HuggingFace Inc. team.
...
def safer_eval(func: Callable):
    ...
    def _check_return(
        ...
        result = func(expression, state, static_tools,
        ...
        if "*" not in authorized_imports:
            if isinstance(result, ModuleType):
                ...
                for qualified_function_name in DANGEROUS_FUNCTIONS:
                    module_name, function_name = qualified_function_name.rsplit(
                        ".", 1)
                    if (
...
def evaluate_attribute(
    ...
    value = evaluate_ast(expression.value, state,
...
def evaluate_unaryop(
    ...
    operand = evaluate_ast(expression.operand, state,
                           static_tools, custom_tools, authorized_imports)
...
def evaluate_class_def(
    ...
    bases = [evaluate_ast(base, state, static_tools, custom_tools,
...
def evaluate_augassign(
    ...
    def get_current_value(target: ast.AST) -> Any:
        if isinstance(target, ast.Name):
            ...
            obj = evaluate_ast(target.value, state, static_tools,
                               custom_tools, authorized_imports)
            key = evaluate_ast(target.slice, state, static_tools,
                               custom_tools, authorized_imports)
            return obj[key]
        elif isinstance(target, ast.Attribute):
            obj = evaluate_ast(target.value, state, static_tools,
...
def evaluate_binop(
    ...
    left_val = evaluate_ast(binop.left, state, static_tools,
                            custom_tools, authorized_imports)
...
def evaluate_assign(
    ...
    result = evaluate_ast(assign.value, state, static_tools,
                          custom_tools, authorized_imports)
    if len(assign.targets) == 1:
        target = assign.targets[0]
        set_value(target, result, state, static_tools,
                  custom_tools, authorized_imports)
    else:
        expanded_values = []
        ...
        for tgt, val in zip(assign.targets, expanded_values):
            set_value(tgt, val, state, static_tools,
                      custom_tools, authorized_imports)
    return result
...
def set_value(
    ...
    if isinstance(target, ast.Name):
        ...
        for i, elem in enumerate(target.elts):
            set_value(elem, value[i], state, static_tools,
                      custom_tools, authorized_imports)
    elif isinstance(target, ast.Subscript):
        obj = evaluate_ast(target.value, state, static_tools,
                           custom_tools, authorized_imports)
...
def evaluate_call(
    ...
    if isinstance(call.func, ast.Call):
        func = evaluate_ast(call.func, state, static_tools,
                            custom_tools, authorized_imports)
    elif isinstance(call.func, ast.Lambda):
        func = evaluate_ast(call.func, state, static_tools,
                            custom_tools, authorized_imports)
    elif isinstance(call.func, ast.Attribute):
        obj = evaluate_ast(call.func.value, state, static_tools,
                           custom_tools, authorized_imports)
    ...
    for arg in call.args:
        if isinstance(arg, ast.Starred):
            args.extend(evaluate_ast(arg.value, state, static_tools,
                        custom_tools, authorized_imports))
        else:
            args.append(evaluate_ast(arg, state, static_tools,
                        custom_tools, authorized_imports))
...
def evaluate_subscript(
    ...
    index = evaluate_ast(subscript.slice, state, static_tools,
                         custom_tools, authorized_imports)
    value = evaluate_ast(subscript.value, state, static_tools,
                         custom_tools, authorized_imports)
    try:
        return value[index]
...
def evaluate_condition(
    ...
    result = True
    left = evaluate_ast(condition.left, state, static_tools,
                        custom_tools, authorized_imports)
    for i, (op, comparator) in enumerate(zip(condition.ops, condition.comparators)):
        op = type(op)
        right = evaluate_ast(comparator, state, static_tools,
                             custom_tools, authorized_imports)
        if op == ast.Eq:
...
def evaluate_for(
    ...
    result = None
    iterator = evaluate_ast(for_loop.iter, state,
                            static_tools, custom_tools, authorized_imports)
...
def evaluate_listcomp(
    ...
    def inner_evaluate(generators: List[ast.comprehension], index: int, current_state: Dict[str, Any]) -> List[Any]:
        ...
        for value in iter_value:
            ...
            if all(
                evaluate_ast(if_clause, new_state, static_tools,
                             custom_tools, authorized_imports)
...
def evaluate_try(
    ...
    try:
        for stmt in try_node.body:
            evaluate_ast(stmt, state, static_tools,
                         custom_tools, authorized_imports)
        ...
        for handler in try_node.handlers:
            if handler.type is None or isinstance(
                e,
                evaluate_ast(handler.type, state, static_tools,
                             custom_tools, authorized_imports),
            ):
                matched = True
                if handler.name:
...
def evaluate_raise(
    ...
    if raise_node.exc is not None:
        exc = evaluate_ast(raise_node.exc, state, static_tools,
                           custom_tools, authorized_imports)
    else:
        exc = None
    if raise_node.cause is not None:
        cause = evaluate_ast(raise_node.cause, state,
                             static_tools, custom_tools, authorized_imports)
    else:
        cause = None
    if exc is not None:
...
def evaluate_assert(
    ...
    if not test_result:
        if assert_node.msg:
            msg = evaluate_ast(assert_node.msg, state,
                               static_tools, custom_tools, authorized_imports)
...
def check_module_authorized(module_name, authorized_imports):
    if "*" in authorized_imports:
        ...
        # Check if the module is directly authorized
        if module_name in authorized_imports:
            return True

        # Check if any parent module is authorized (this allows submodules)
        module_path = module_name.split(".")
        # ["A", "B", "C"] -> ["A", "A.B", "A.B.C"]
        module_subpaths = [".".join(module_path[:i])
                           for i in range(1, len(module_path) + 1)]

        # Check if any parent module is in the authorized imports
        for auth_import in authorized_imports:
            # If the authorized import is a parent of the requested module
...
def evaluate_import(expression, state, authorized_imports):
    if isinstance(expression, ast.Import):
        ...
        if check_module_authorized(expression.module, authorized_imports):
            raw_module = __import__(expression.module, fromlist=[
            ...
            if expression.names[0].name == "*":  # Handle "from module import *"
                ...
                # If no __all__, import all public names (those not starting with '_')
                else:
...
def evaluate_ast(
    ...
    if isinstance(expression, ast.Assign):
        ...
        return slice(
            evaluate_ast(
                expression.lower, *common_params) if expression.lower is not None else None,
            evaluate_ast(
                expression.upper, *common_params) if expression.upper is not None else None,
            evaluate_ast(
                expression.step, *common_params) if expression.step is not None else None,
        )
        ...
        raise ReturnException(evaluate_ast(
            expression.value, *common_params) if expression.value else None)
...
def evaluate_python_code(
    ...
    # Add basic Python tools including print to static_tools
    for name, tool in BASE_PYTHON_TOOLS.items():
